...
?>
    <div class="container">
                ...
                <?php if(isset($upcoming_events)){ ?>
                    <?php foreach ($upcoming_events as $upcoming_event){ ?>
                            <?php if($_SESSION['lang'] == 'en_GB'){ ?>
                                <a href="/views/event/event.php?id=<?= $upcoming_event['id_event'];?>"><?= (new \DateTime($upcoming_event['start']))->format('m/d/Y H:i') ?></a></br>
                            <?php }else{ ?>
                                <a href="/views/event/event.php?id=<?= $upcoming_event['id_event'];?>"><?= (new \DateTime($upcoming_event['start']))->format('d/m/Y H:i') ?></a></br>
                            <?php } ?>
                    <?php } ?>
                <?php }else{ ?>
            ...
            <?php if($users_events){ ?>
                <?php if(isset($past_events)){ ?>
                    <?php foreach ($past_events as $past_event){ ?>
                        <?php if($_SESSION['lang'] == 'en_GB'){ ?>
                            <a href="/views/event/event.php?id=<?= $past_event['id_event'];?>"><?= (new \DateTime($past_event['start']))->format('m/d/Y H:i') ?></a></br>
                        <?php }else{ ?>
                            <a href="/views/event/event.php?id=<?= $past_event['id_event'];?>"><?= (new \DateTime($past_event['start']))->format('d/m/Y H:i') ?></a></br>
                        <?php } ?>
                    <?php } ?>
                <?php }else{ ?>
        ...
        <div> <a class="btn btn-primary mt-5" href="/views/event/add.php"><?= Translation::of('createAppointementTitle') ?></a></div>
        <?php if($_SESSION['auth']->getIdRole() == 1){ ?>
            <div> <a class="btn btn-primary mt-5" href="/views/event/add.php"><?= Translation::of('return') ?></a></div>
        <?php }else{ ?>
